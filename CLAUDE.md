...
All three require `--annotations_csv <vindr-mammo/finding_annotations.csv>`, default to `--split training`, set `input_nc=1 output_nc=1 preprocess=none`, and accept `--bilateral_size H W` (default `512 384`) and `--flip_right`. `scheduled_bilateral` adds `--pair_schedule "epochs:p,epochs:p,..."` (e.g. `"50:0.8,50:0.5,100:0.0"`); past the end of the schedule, `p` holds at its last value. The schedule is driven by `dataset.current_epoch`, which `train.py` updates each epoch via `set_epoch`.
